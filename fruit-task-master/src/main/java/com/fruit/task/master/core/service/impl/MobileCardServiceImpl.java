...
import com.fruit.task.master.core.mapper.MobileCardMapper;
import com.fruit.task.master.core.model.mobilecard.MobileCardModel;
import com.fruit.task.master.core.service.MobileCardService;
...
public class MobileCardServiceImpl<T> extends BaseServiceImpl<T> implements MobileCardService<T> {
    ...
    public MobileCardModel getMobileCard(MobileCardModel model, int isCache) throws Exception {
        ...
        if (isCache == ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_ZERO){
            String strKeyCache = CachedKeyUtils.getCacheKey(TkCacheKey.MOBILE_CARD_PHONE_NUM, model.getPhoneNum());
