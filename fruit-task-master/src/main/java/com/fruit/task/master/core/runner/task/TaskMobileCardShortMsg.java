...
//        log.info("----------------------------------TaskMobileCardShortMsg.handle()----start");
        // 获取手机短信数据
        StatusModel statusQuery = TaskMethod.assembleTaskStatusQuery(limitNum, 1, 0, 0,1,0);
        List<MobileCardShortMsgModel> synchroList = ComponentUtil.taskMobileCardShortMsgService.getDataList(statusQuery);
        for (MobileCardShortMsgModel data : synchroList){
            try{
                // 锁住这个数据流水
                        ...
                        if (data.getMobileCardId() != null && data.getMobileCardId() > 0){
                            ...
                            if (shortMsgArrearsModel != null){
                                ...
                                if (num > 0){
                                    ...
                                    statusModel = TaskMethod.assembleTaskUpdateStatus(data.getId(), 2, 0, 0, "添加数据响应行为0");
                                }
                            }else {
                                statusModel = TaskMethod.assembleTaskUpdateStatus(data.getId(), 2, 0, 0, "必要字段数据为空");
                            }
                        }else {
                            statusModel = TaskMethod.assembleTaskUpdateStatus(data.getId(), 2, 0, 0, "手机卡的ID为空");
                        }
...
//                log.info("----------------------------------TaskMobileCardShortMsg.handle()----end");
                ...
                e.printStackTrace();
                // 更新此次task的状态：更新成失败：因为必填项没数据
                StatusModel statusModel = TaskMethod.assembleTaskUpdateStatus(data.getId(), 2, 0,0, "异常失败try!");
                ComponentUtil.taskMobileCardShortMsgService.updateStatus(statusModel);
            }
        }
